#!/usr/bin/env python3
"""
🦞⚡ NeoClaw Workspace Sync
Multi-backend backup: HuggingFace Datasets, GitHub Gist, S3
"""
...
BACKUP_DATASET = os.environ.get("BACKUP_DATASET_NAME", "neoclaw-backup")
SYNC_INTERVAL = int(os.environ.get("SYNC_INTERVAL", "180"))
GIT_USER = os.environ.get("WORKSPACE_GIT_USER", "neoclaw@example.com")
GIT_NAME = os.environ.get("WORKSPACE_GIT_NAME", "NeoClaw Bot")
...
BACKUP_DIR = Path("/tmp/neoclaw-backup")
STATE_FILE = Path("/tmp/neoclaw-sync-state.json")
...
class HFBackend:
    def __init__(self):
        self.repo_id = f"{HF_USERNAME}/{BACKUP_DATASET}"
    ...
    def restore(self) -> bool:
        ...
        try:
            ...
            log(f"Restoring from HF dataset: {self.repo_id}", "SYNC")
            snapshot_download(
    ...
    def sync(self) -> bool:
        ...
        try:
            ...
            # Copy workspace (excluding large/unnecessary files)
            ws = Path(WORKSPACE_PATH)
            ...
            meta = {
                ...
                "platform": os.environ.get("NEOCLAW_PLATFORM", "unknown"),
                "model": os.environ.get("LLM_MODEL", "unknown"),
                "hash": get_workspace_hash()
            }
            (self.local_dir / ".neoclaw-meta.json").write_text(json.dumps(meta, indent=2))
            ...
            api.upload_folder(
                ...
                repo_type="dataset",
                token=HF_TOKEN,
                commit_message=f"NeoClaw auto-sync {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}"
            )
...
class GistBackend:
    ...
    def sync(self) -> bool:
        ...
        try:
            import urllib.request
            # Collect workspace files (small ones only for Gist)
            files = {}
            ws = Path(WORKSPACE_PATH)
            for f in sorted(ws.rglob("*")):
                if f.is_file() and f.stat().st_size < 1_000_000:  # <1MB per file
            ...
            payload = json.dumps({
                "description": f"NeoClaw backup {datetime.now(timezone.utc).isoformat()}",
                "files": files
            }).encode()
            ...
            with urllib.request.urlopen(req) as resp:
                ...
                log(f"Gist sync complete! {result.get('html_url', '')}", "OK")
            return True
        except Exception as e:
...
# ─── Git Fallback Backend ─────────────────────────────────────
class GitBackend:
    def __init__(self):
        self.repo_url = f"https://huggingface.co/datasets/{HF_USERNAME}/{BACKUP_DATASET}" \
            if HF_USERNAME else None

    def restore(self) -> bool:
        if not self.repo_url:
            return False
        try:
            log("Restoring via Git clone...", "SYNC")
            git_dir = BACKUP_DIR / "git"
            if git_dir.exists():
                shutil.rmtree(git_dir)

            subprocess.run(
                ["git", "clone", "--depth=1", self.repo_url, str(git_dir)],
            ...
            if git_dir.exists() and any(git_dir.iterdir()):
                for item in git_dir.iterdir():
                    ...
                    if item.is_dir():
                        ...
                        shutil.copy2(item, dest)
                log("Git restore complete!", "OK")
                return True
        except Exception as e:
            log(f"Git restore failed: {e}", "WARN")
        return False
    ...
    def sync(self) -> bool:
        ...
        try:
            ...
            if not git_dir.exists():
                subprocess.run(
                    ["git", "clone", self.repo_url, str(git_dir)],
                    capture_output=True, timeout=60
                )

            # Copy workspace to git dir
            ws = Path(WORKSPACE_PATH)
            for item in ws.iterdir():
            ...
            # Commit and push
            subprocess.run(["git", "config", "user.email", GIT_USER], cwd=git_dir, capture_output=True)
            subprocess.run(["git", "config", "user.name", GIT_NAME], cwd=git_dir, capture_output=True)
            ...
            if result.returncode != 0:  # There are changes
                subprocess.run(
                    ["git", "commit", "-m", f"NeoClaw sync {datetime.now(timezone.utc).isoformat()}"],
                    cwd=git_dir, capture_output=True
                )
                subprocess.run(["git", "push"], cwd=git_dir, capture_output=True, timeout=30)
                log("Git sync complete!", "OK")
            else:
                log("No changes to sync", "INFO")
            return True
        except Exception as e:
            log(f"Git sync failed: {e}", "WARN")
            return False
...
class SyncManager:
    def __init__(self):
        ...
        # Git fallback always available if HF creds exist
        if HF_USERNAME:
            self.backends.append(GitBackend())
    ...
    def sync(self) -> bool:
        ...
        for backend in self.backends:
            if backend.sync():
                success = True
                break  # First successful backend is enough
        return success
